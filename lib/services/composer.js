...
function resolveComponentReferences(data, locals, filter = referenceProperty, uri) {
  ...
  return schemaPromise.then(schema => {
    ...
    return bluebird.each(orderedRefs, ([label, referenceObject]) => {
      ...
      return components.get(ref, locals)
        ...
        .catch(function (error) {
          ...
          if (isNotFound && label.includes('content') && !ref.includes('@published')) {
            log('info', `URI ${ref} not found, creating...`);

            return components.put(ref, {}, locals)
              .then((obj) => {
                _.assign(referenceObject, _.omit(obj, referenceProperty));
              })
          }
